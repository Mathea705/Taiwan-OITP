...
)
{
    ...
    const float radius0 = waterScale0;
    float sliceNumber = log2( max( taxicab / radius0, 1.0 ) );
    sliceNumber = clamp(sliceNumber, minSlice, maxSlice + 1);

    lodAlpha = frac(sliceNumber);

    uint extent = floor(sliceNumber);

    sliceNumber = min(sliceNumber, maxSlice);
    ...
    if (extent == (uint)g_Crest_LodCount)
    {
        lodAlpha = 1.0;
    }
}
